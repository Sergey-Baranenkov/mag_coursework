# Свертка и по фичам и по времени
from torch import nn


def calculate_shape_features(x, n_layers=4):
    for i in range(n_layers):
        x = (x - 2) / 2

    return int(x)

def calculate_shape_time(x, n_layers=4):
    for i in range(n_layers):
        x = (x - 4) / 2
...
class Conv3Model(nn.Module):
    def __init__(self, num_class, time_size, feature_size, n_conv_layers = 4):
        super(Conv3Model, self).__init__()

        self.conv_layers = nn.ModuleList()
        self.conv_layers.append(conv_block(1, 16, (5, 3), (2, 2)))
        self.conv_layers.append(conv_block(16, 32, (5, 3), (2, 2)))

        for i in range(n_conv_layers - 2):
            self.conv_layers.append(conv_block(32, 32, (5, 3), (2, 2)))

        self.linear_1 = linear_block(32 * calculate_shape_time(time_size, n_conv_layers) * calculate_shape_features(feature_size, n_conv_layers), 256)

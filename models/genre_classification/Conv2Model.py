...
class Conv2Model(nn.Module):
    def __init__(self, num_class, feature_size, time_size):
        super(Conv2Model, self).__init__()

        vertical_size = int(feature_size / 4)

        self.conv1 = nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(1, vertical_size))
        self.conv2 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(1, vertical_size))
        self.conv3 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(1, vertical_size))
        self.conv4 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(1, vertical_size))

        self.linear_1 = nn.Linear(4 * time_size * 128, 512)

...
from .dataset import DecadeClassificationDatasetForExternal, DecadeClassificationDataset
from ..genre_classification import FeatureNormalizer, FeatureLabelMerger
...
def feature_preparator(feature_file: str, batch_size: int, transform=None, normalize=True, external=False):
    ...
    metadata = pd.read_csv('decade_classification/decade_classification_metadata.csv')

    # Создаем словарь маппер id песни - декада
    id_to_decade = {str(k): v for k, v in metadata.set_index('track_id').to_dict()['prep_decade'].items()}

    # Мержим метки и фичи между собой. Примечательно, что сохраняется только пересечение id, которые есть в обоих словарях.
    merged_feature_label = FeatureLabelMerger.merge(id_to_features, id_to_decade)
    ...
    dataset = DecadeClassificationDataset(merged_feature_label, label_to_idx, transform=transform) if not external \
        else DecadeClassificationDatasetForExternal(merged_feature_label, label_to_idx, transform=transform)
